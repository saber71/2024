 ...
 */
export interface ICollection<V extends FilterItem = FilterItem> {
  /**
   * 集合的名称。
   */
   ...
   * @returns 返回符合条件的项的数组。
   */
  search(condition?: FilterCondition<V>): V[] | Promise<V[]>
   ...
   */
  searchOne(condition?: FilterCondition<V>): Promise<V | undefined> | V | undefined
   ...
   * @returns 返回被删除的项的数组。
   */
  delete(condition: FilterCondition<V>): Promise<V[]> | V[]
}
