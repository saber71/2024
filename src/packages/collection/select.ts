import type { FilterCondition, FilterItem } from "@packages/filter"
...
export class Select<Result extends FilterItem, FromValue extends FilterItem> {
   ...
   * @param fromCollection 来源数据集合。
   */
  static from<Result extends FilterItem, FromValue extends FilterItem>(fromCollection: ICollection<FromValue>) {
   ...
   * @returns 返回 Select 实例，支持链式调用。
   */
  join<V extends FilterItem>(collection: ICollection<V>, cb: (item: FromValue) => Promise<V>) {
   ...
   * @returns 返回 Select 实例，支持链式调用。
   */
  expose<V extends FilterItem>(collection: ICollection<V>, cb: (value: V) => Partial<Result>) {
